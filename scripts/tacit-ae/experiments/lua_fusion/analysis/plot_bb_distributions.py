...
def main():
    ...
    for idx, (ax, r) in enumerate(zip(axes, b.itertuples())):
        ...
        # title: the block's address range; the execution count goes in the caption
        ax.set_title(f'{r.bb}', loc='left', color=INK, pad=3)
        ax.text(-0.18, 1.14, string.ascii_lowercase[idx], transform=ax.transAxes,
                fontsize=st['panel'], fontweight='bold', va='top', ha='left', color=INK)
        ax.set_xlim(0, xmax)
        ax.xaxis.set_major_locator(mpl.ticker.MaxNLocator(nbins=6, integer=True))
        ax.set_ylim(0, max(share) * 1.18)
        ax.spines[['top', 'right']].set_visible(False)
        ax.tick_params(length=2.5, pad=1.5)
        if idx % cols == 0:
            ax.set_ylabel('Share (%)')
        # x label on the last row only; the axes are not shared (one block's slow mode sits
        # near 68 cycles while another's whole range is 2-10) but the unit is the same
        if idx + cols >= n:            # no panel below this one
    ...
    handles.append(plt.Line2D([], [], color=BAR, lw=3.2, label='latency'))
