...
later arm (e.g. a control binary) keeps its legend entry so rows stay comparable.
  ...
  plot_pred_grid.py --targets MUL,ADD --out fig.pred_grid \
      "baseline=base.dispatch_hist.csv:configs/optab_base.json" \
      "+MUL->MUL=g1.dispatch_hist.csv:configs/optab_mulmul.json" \
      "+MUL->ADD=g2.dispatch_hist.csv:configs/optab_mmadd.json"
"""
import argparse
import json
import pathlib
...
def main():
    ...
    ap.add_argument('--row-height', type=float, default=34.0, help='mm per row')
    ap.add_argument('--font-scale', type=float, default=1.25,
                    help='text size multiplier over the venue base')
    ...
    for r, tgt in enumerate(targets):
        ...
        for c, (label, d, tab, rev) in enumerate(arms):
            ...
            # headroom scales with the legend the row has to hold (one line per predecessor)
            ax.set_ylim(0, _rowmax(grid, tgt, arms, denom, xmax) * (1.18 + 0.18 * max(0, len(key) - 2)))
            ax.spines[['top', 'right']].set_visible(False)
            ax.set_axisbelow(True)
            ax.yaxis.grid(True, color=GRID, linewidth=st['grid_lw'])
            if r == 0:
                ax.set_title(label, loc='center', color=INK, pad=7)
            if c == 0:
            ...
            # the instance count is the same in every arm (the canary guarantees it) and
            # belongs in the caption, not in six panels
            ax.text(-0.14 if c == 0 else -0.06, 1.12, 'abcdefghi'[r * nc + c],
                    transform=ax.transAxes, fontsize=st['panel'], fontweight='bold',
                    va='top', ha='left')
        ...
        if any(p not in key for arm in arms for p in grid[(tgt, arm[0])][1]):
            h.append(Patch(facecolor=NEUTRAL, edgecolor='white', linewidth=0.3,
                           label='other'))
        # the legend goes in whichever panel of the row has the most empty area above its
        # bars in the upper-right quadrant (the baseline's mispredict cluster sits low,
        # so its panel is usually the free one)
        best, room = None, -1.0
        for c, (label, *_) in enumerate(arms):
            ax = axes[r][c]; n, ctx = grid[(tgt, label)]
            xmax_c = xmaxes[tgt]; tot = np.zeros(xmax_c + 1)
            for cyc, w in ctx.values():
                m = cyc <= xmax_c
                np.add.at(tot, cyc[m].astype(int), w[m])
            right = tot[int(xmax_c * 0.45):] * 100 / denom
            free = ax.get_ylim()[1] - right.max()
            if free > room:
                best, room = c, free
        # anchor the legend's BOTTOM edge a little above the x axis: its last entry then
        # clears the axis and the residual bars, and it grows upward into the free space
        axes[r][best].legend(handles=h, loc='lower right', bbox_to_anchor=(1.02, 0.08),
